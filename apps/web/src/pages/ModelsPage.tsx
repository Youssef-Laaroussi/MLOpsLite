...
import { Box, ArrowUpCircle, BarChart3, Layers } from "lucide-react";
            ...
            {models.length === 0 ? (
              ...
              models.map((m, idx) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition">
                  ...
                  <td className="px-6 py-4 font-mono text-xs">v{m.version}</td>
                  <td className="px-6 py-4">
                    <StatusBadge status={m.stage || "DEVELOPMENT"} />
                  </td>
                  <td className="px-6 py-4 text-xs font-mono text-slate-400">
                    {m.mlflow_run_id ? (
                      <span className="text-indigo-400">{m.mlflow_run_id.slice(0, 8)}...</span>
                    ) : (
                      "Manual"
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-sky-950/80 text-sky-400 hover:bg-sky-900 border border-sky-800/80 text-xs font-medium transition"
                    >
      ...
      {selectedModel && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 space-y-4">
            <h3 className="text-lg font-bold text-white">Promote Model Stage</h3>
            <p className="text-sm text-slate-400">
            ...
            {targetStage === "PRODUCTION" && (
              <div className="p-3 bg-amber-950/40 border border-amber-800/80 rounded-lg text-xs text-amber-300">
                ⚠️ Promoting to Production will automatically archive any currently active production version of this model.
              </div>
            )}
